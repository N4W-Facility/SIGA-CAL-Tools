function UserData = Read_Boundary(FilePath)
% Función para lectura de archivo de condiciones de frontera - SIGA-CAL 2022
% -------------------------------------------------------------------------
...
try
    while ischar(LineText)
        ...
        elseif i == 4
            % Almacenamiento
            nn = 17;
            UserData.Data = cell(UserData.N,nn);
            for j = 1:UserData.N
                % Leer nueva línea
                LineText = fgetl(ID_File);
                
                Tmp = strsplit(LineText,' ');
                UserData.Data(j,:) = Tmp(1:nn);                
            end
